...
//   var noOfPages = data.length/document.getElementById("rowSelection").value

// console.log(noOfPages)

//   var startingIndexForEachPage = (pageNo * 10) - 10 + 1;

//   var endingIndexForEachPage = (pageNo * 10);
function defaultRun() {
  
  var startingIndexForEachPage = 1

  var endingIndexForEachPage = 10

  for (i = startingIndexForEachPage - 1; i <= endingIndexForEachPage - 1; i++) {
    var rowData = document.createElement("tr");
    var tabeldata = document.createElement("td");
    tabeldata.innerText = data[i].id;
    rowData.appendChild(tabeldata);
    tabeldata = document.createElement("td");
    tabeldata.innerText = data[i].name;
    rowData.appendChild(tabeldata);
    tabeldata = document.createElement("td");
    tabeldata.innerText = data[i].email;
    rowData.appendChild(tabeldata);
...
function generateButtons() {
  ...
  var noOfButtons = data.length / numberOfRecordsPerPage;

  var buttonsElement = document.getElementById("buttons");
  buttonsElement.innerHTML = "";

  var firstElement = document.createElement("button");
  firstElement.setAttribute("value", 1);
  firstElement.innerText = "First";
   firstElement.addEventListener("click", records);
  buttonsElement.appendChild(firstElement);
  ...
  for (var i = 1; i <= noOfButtons; i++) {
    ...
    buttonElement.addEventListener("click", records);
...
function records(t) {
  ...
  var e = document.getElementById("rowSelection");
  var numberOfRecordsPerPage = parseInt(e.value);
  var buttons = parseInt(t.target.value);

  var startingIndexForEachPage =
    buttons * numberOfRecordsPerPage - numberOfRecordsPerPage + 1;
  var endingIndexForEachPage = buttons * numberOfRecordsPerPage;
